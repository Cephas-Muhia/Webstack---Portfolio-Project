...
function Catalogue() {
  ...
  const handleCustomizeClick = async (cakeId, flavor) => {
    try {
        // Convert the cake ID to an ObjectId
        const objectId = mongoose.Types.ObjectId(cakeId); // Ensure the ID is a valid ObjectId

        // Prepare the data for the order
        const orderPayload = {
            cake: objectId,  // Send the selected cake ID as an ObjectId
